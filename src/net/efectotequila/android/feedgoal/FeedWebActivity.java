...
public class FeedWebActivity extends Activity {
	...
	private class ItemWebViewClient extends WebViewClient {
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
			...
			if (url != null && url.indexOf("market://") == 0) {
				try {
					TrackerAnalyticsHelper.trackEvent(FeedWebActivity.this,
							LOG_TAG, "Android Market URL", url, 1);
					startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url)));
					return true;
				} catch (Exception e) {
					Log.e(LOG_TAG, "", e);
					errorId = errorId + 1;
					TrackerAnalyticsHelper.trackError(FeedWebActivity.this,
							Long.toString(errorId), e.getMessage(), LOG_TAG);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		webview.setOnLongClickListener(new OnLongClickListener() {
			public boolean onLongClick(View v) {
				...
				AlertDialog.Builder alertListBuilder = new AlertDialog.Builder(
						FeedWebActivity.this);
				alertListBuilder.setTitle(R.string.ctx_menu_title).setIcon(
						R.drawable.ic_dialog);
				alertListBuilder.setItems((CharSequence[]) options
						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								CharSequence option = (CharSequence) options
										.get(optionId);
								if (option.equals(getString(R.string.add_fav))) {
									TrackerAnalyticsHelper.trackEvent(
											FeedWebActivity.this, LOG_TAG,
											"OptionDialog_AddFavorite", item
													.getLink().toString(), 1);
									// item.favorite();
									...
									values.put(
											...
											DbSchema.ON);
									mDbFeedAdapter.updateItem(mItemId, values,
											null);
									Toast.makeText(FeedWebActivity.this,
											R.string.add_fav_msg,
											Toast.LENGTH_SHORT).show();
								} else if (option
										.equals(getString(R.string.remove_fav))) {
									TrackerAnalyticsHelper.trackEvent(
											FeedWebActivity.this, LOG_TAG,
											"OptionDialog_RemoveFavorite", item
													.getLink().toString(), 1);
									// item.unfavorite();
									Date now = new Date();
									long diffTime = now.getTime()
											- item.getPubdate().getTime();
									long maxTime = SharedPreferencesHelper
											.getPrefMaxHours(FeedWebActivity.this) * 60 * 60 * 1000; // Max
																										// hours
																										// expressed
																										// in
																										// milliseconds
									...
									if (maxTime > 0 && diffTime > maxTime) {
										AlertDialog.Builder builder = new AlertDialog.Builder(
												FeedWebActivity.this);
										builder.setMessage(
												R.string.remove_fav_confirmation)
												.setCancelable(false)
												.setPositiveButton(
														R.string.yes,
														new DialogInterface.OnClickListener() {
										...
										.equals(getString(R.string.next_item))) {
									TrackerAnalyticsHelper.trackEvent(
											FeedWebActivity.this, LOG_TAG,
											"OptionDialog_NextItem", item
													.getLink().toString(), 1);
									feedId = mDbFeedAdapter
											.getItemFeedId(mItemId);
									intent = new Intent(FeedWebActivity.this,
											FeedWebActivity.class);
									intent.putExtra(DbSchema.ItemSchema._ID,
											mDbFeedAdapter.getNextItemId(
													feedId, mItemId));
									startActivity(intent);
									finish();
								} else if (option
										.equals(getString(R.string.previous_item))) {
									TrackerAnalyticsHelper.trackEvent(
											FeedWebActivity.this, LOG_TAG,
											"OptionDialog_PreviousItem", item
													.getLink().toString(), 1);
									feedId = mDbFeedAdapter
											.getItemFeedId(mItemId);
									intent = new Intent(FeedWebActivity.this,
											FeedWebActivity.class);
									intent.putExtra(DbSchema.ItemSchema._ID,
											mDbFeedAdapter.getPreviousItemId(
													feedId, mItemId));
									startActivity(intent);
									finish();
								} else if (option
										.equals(getString(R.string.share))) {
									if (item != null) {
										TrackerAnalyticsHelper.trackEvent(
												FeedWebActivity.this, LOG_TAG,
												"OptionDialog_Share", item
														.getLink().toString(),
												1);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_opt_home:
			TrackerAnalyticsHelper.trackEvent(this, LOG_TAG, "OptionMenu_Home",
					"Home", 1);
			// Kill the FeedTabActivity that started this FeedWebActivity,
		...
		default:
			if (item.getGroupId() == SharedPreferencesHelper.CHANNEL_SUBMENU_GROUP) {
				...
				// (won't be the initial FeedChannelActivity channel id) if back
				// button is pressed
				setResult(RESULT_OK);
				startActivity(item.getIntent());
				// finish();
	 ...
	 * 
	 * if (v.getId() == R.id.webview) { menu.setHeaderTitle
	 * (R.string.ctx_menu_title); MenuInflater inflater = getMenuInflater();
	 * 
	 * Item item = mDbFeedAdapter.getItem(mItemId);
	 * 
	 * if (item != null) { long feedId = mDbFeedAdapter.getItemFeedId(mItemId);
	 * boolean isFirstItem = false; boolean isLastItem = false; if (mItemId ==
	 * mDbFeedAdapter.getFirstItem(feedId).getId()) isFirstItem = true; else if
	 * (mItemId == mDbFeedAdapter.getLastItem(feedId).getId()) isLastItem =
	 * true;
	 * 
	 * if (item.isFavorite()) { if (isFirstItem)
	 * inflater.inflate(R.menu.ctx_menu_item_online_notfav_next, menu); else if
	 * (isLastItem) inflater.inflate(R.menu.ctx_menu_item_online_notfav_prev,
	 * menu); else
	 * inflater.inflate(R.menu.ctx_menu_item_online_notfav_next_prev, menu); }
	 * else { if (isFirstItem)
	 * inflater.inflate(R.menu.ctx_menu_item_online_fav_next, menu); else if
	 * (isLastItem) inflater.inflate(R.menu.ctx_menu_item_online_fav_prev,
	 * menu); else inflater.inflate(R.menu.ctx_menu_item_online_fav_next_prev,
	 * menu); } } } }
	 * 
	 * 
	 * public boolean onContextItemSelected(MenuItem menuItem) { Item item =
	 * mDbFeedAdapter.getItem(mItemId); ContentValues values = null; Intent
	 * intent = null; long feedId = -1;
	 * 
	 * switch (menuItem.getItemId()) { case R.id.add_fav:
	 * TrackerAnalyticsHelper.
	 * trackEvent(this,LOG_TAG,"ContextMenu_AddFavorite",item
	 * .getLink().toString(),1); //item.favorite(); values = new
	 * ContentValues(); values.put(DbSchema.ItemSchema.COLUMN_FAVORITE,
	 * DbSchema.ON); mDbFeedAdapter.updateItem(mItemId, values, null);
	 * Toast.makeText(this, R.string.add_fav_msg, Toast.LENGTH_SHORT).show();
	 * return true; case R.id.remove_fav:
	 * TrackerAnalyticsHelper.trackEvent(this,
	 * LOG_TAG,"ContextMenu_RemoveFavorite",item.getLink().toString(),1);
	 * //item.unfavorite(); Date now = new Date(); long diffTime = now.getTime()
	 * - item.getPubdate().getTime(); long maxTime =
	 * SharedPreferencesHelper.getPrefMaxHours(this) * 60 * 60 * 1000; // Max
	 * hours expressed in milliseconds // test if item has expired if (maxTime >
	 * 0 && diffTime > maxTime) { AlertDialog.Builder builder = new
	 * AlertDialog.Builder(this);
	 * builder.setMessage(R.string.remove_fav_confirmation)
	 * .setCancelable(false) .setPositiveButton(R.string.yes, new
	 * DialogInterface.OnClickListener() { public void onClick(DialogInterface
	 * dialog, int id) { ContentValues values = new ContentValues();
	 * values.put(DbSchema.ItemSchema.COLUMN_FAVORITE, DbSchema.OFF);
	 * mDbFeedAdapter.updateItem(mItemId, values, null);
	 * Toast.makeText(FeedWebActivity.this, R.string.remove_fav_msg,
	 * Toast.LENGTH_SHORT).show(); } }) .setNegativeButton(R.string.no, new
	 * DialogInterface.OnClickListener() { public void onClick(DialogInterface
	 * dialog, int id) { dialog.cancel(); } }); builder.create().show(); } else
	 * { values = new ContentValues();
	 * values.put(DbSchema.ItemSchema.COLUMN_FAVORITE, DbSchema.OFF);
	 * mDbFeedAdapter.updateItem(mItemId, values, null); Toast.makeText(this,
	 * R.string.remove_fav_msg, Toast.LENGTH_SHORT).show(); } return true; case
	 * R.id.next:
	 * TrackerAnalyticsHelper.trackEvent(this,LOG_TAG,"ContextMenu_NextItem"
	 * ,item.getLink().toString(),1); feedId =
	 * mDbFeedAdapter.getItemFeedId(mItemId); intent = new Intent(this,
	 * FeedWebActivity.class); intent.putExtra(DbSchema.ItemSchema._ID,
	 * mDbFeedAdapter.getNextItemId(feedId, mItemId)); startActivity(intent);
	 * finish(); return true; case R.id.previous:
	 * TrackerAnalyticsHelper.trackEvent
	 * (this,LOG_TAG,"ContextMenu_PreviousItem",item.getLink().toString(),1);
	 * feedId = mDbFeedAdapter.getItemFeedId(mItemId); intent = new Intent(this,
	 * FeedWebActivity.class); intent.putExtra(DbSchema.ItemSchema._ID,
	 * mDbFeedAdapter.getPreviousItemId(feedId, mItemId));
	 * startActivity(intent); finish(); return true; case R.id.share: if (item
	 * != null) {
	 * TrackerAnalyticsHelper.trackEvent(this,LOG_TAG,"ContextMenu_Share"
	 * ,item.getLink().toString(),1); Intent shareIntent = new
	 * Intent(Intent.ACTION_SEND); shareIntent.putExtra(Intent.EXTRA_SUBJECT,
	 * String.format(getString(R.string.share_subject),
	 * getString(R.string.app_name))); shareIntent.putExtra(Intent.EXTRA_TEXT,
	 * item.getTitle() + " " + item.getLink().toString());
	 * shareIntent.setType("text/plain");
	 * startActivity(Intent.createChooser(shareIntent,
	 * getString(R.string.share))); } return true; default: return
